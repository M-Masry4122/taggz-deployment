...
def build_database():
    ...
    try:
        ...
        poor_quality_images = [os.path.basename(img) for img in face_recognition.poor_quality_images]
        no_face_images = [os.path.basename(img) for img in face_recognition.no_face_images]
...
def compare_faces():
    ...
    try:
        ...
        if not data or 'profile_picture' not in data or 'event_name' not in data:
            return jsonify({'error': 'Profile picture and event name are required'}), 400
        ...
        if not profile_picture.startswith(app.config['PROFILE_PICTURES_FOLDER']):
            profile_picture = os.path.join(app.config['PROFILE_PICTURES_FOLDER'], profile_picture)
        ...
        print(f'match_count: {len(results)}, time_taken: {end_time - start_time:.2f} seconds')
...
def upload_profile_picture():
    ...
    try:
        ...
        if not allowed_file(file.filename):
            return jsonify({'error': f'File type not allowed. Use {", ".join(app.config["ALLOWED_EXTENSIONS"])}'}), 400
...
def health_check():
    return jsonify({
        ...
        'api_version': '1.0',
        'system_info': {
            'profile_pictures': len(os.listdir(app.config['PROFILE_PICTURES_FOLDER'])),
