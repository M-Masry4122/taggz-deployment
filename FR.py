...
class EnhancedFaceRecognition:
    ...
    def _load_and_preprocess_image(self, image_path: Union[str, Path]) -> Optional[np.ndarray]:
    ...
    def rebuild_face_database(self, image_folder: Union[str, Path]) -> Dict:
        ...
        if cache_file:
            ...
            try:
                ...
                logger.info(f"Successfully updated cache for {os.path.basename(os.path.normpath(str(image_folder)))}")
            except Exception as e:
    ...
    def load_face_database(self, image_folder: Union[str, Path], cache: bool = True) -> Dict:
        ...
        image_paths = [os.path.join(str(image_folder), f) for f in os.listdir(str(image_folder))
        ...
        with ThreadPoolExecutor() as executor:
            ...
            future_to_path = {executor.submit(self._process_single_image, path): path for path in image_paths}
        ...
        if not cache or not cache_file or not cache_file.exists():
            try:
                ...
                if event_id:
                    ...
                    payload = {
                        "no_uploaded": len(database)
                    }
                    response = requests.post(url, json=payload)
                    if response.status_code == 200:
                        logger.info(f"Successfully sent notification for {len(database)} uploaded images")
                    else:
    ...
    def find_matching_faces(
        ...
        for image_path, data in tqdm(face_database.items(), desc="Matching faces", unit="image"):
        ...
        logger.info(f"Found {len(matches)} matches with confidence > {self.similarity_threshold}")
        return matches
    
    def export_match_results(self, matches: List[FaceMatch], output_path: Union[str, Path]) -> None:
        ...
        with open(output_path, 'w') as f:
            ...
            for match in matches:
                f.write(f"{match.image_path},{match.confidence:.4f},{match.face_area}\n")
    ...
    def visualize_matches(
        ...
        for i, match in enumerate(matches[:max_matches]):
            ...
            # Place in grid
            grid[row*height:(row+1)*height, col*width:(col+1)*width] = face_img_resized
...
if __name__ == "__main__":
    ...
    face_recognition.export_match_results(
        matches=matches,
        output_path="./results/matches.csv"
    )
